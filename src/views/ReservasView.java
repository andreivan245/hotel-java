...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtValor.setBounds(65, 328, 63, 33);
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				long diffEmMiliSeg = txtDataS.getDate().getTime() - txtDataE.getDate().getTime();
				long diff = TimeUnit.DAYS.convert(diffEmMiliSeg, TimeUnit.MILLISECONDS);
				if (diff < 0) {
					JOptionPane.showInternalMessageDialog(null, "DATA DE CHECK OUT INVÁLIDA, POR FAVOR ALTERA A DATA!",
							"ERROR", 0);
				}
				...
				if (diff == 0) {
					...
					txtValor.setText(Long.toString(diff * diaria));
				}
					...
					public void run() {
						if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
							...
							if (Integer.parseInt(txtValor.getText()) < 0) {
								JOptionPane.showInternalMessageDialog(null, "VALOR DE RESERVA INVÁLIDO!", "ERROR", 0);
								dispose();
								return;
							}
							...
							String dataReservaIn = formatter.format(txtDataE.getDate());
							String dataReservaOut = formatter.format(txtDataS.getDate());

							reserva.setDataEntrada(dataReservaIn);
							reserva.setDataSaida(dataReservaOut);
							reserva.setValor(Double.parseDouble(txtValor.getText()));
							reserva.setFormaPagamento((String) txtFormaPagamento.getSelectedItem());
							...
							JOptionPane.showMessageDialog(null,
									"Reserva de número " + reservaDAO.ultimoIdReserva() + " feita com sucesso!",
									"Registro Reserva", JOptionPane.DEFAULT_OPTION);
